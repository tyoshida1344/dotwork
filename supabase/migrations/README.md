...
- **DDL を足すときは、既存データへの波及を必ず確認する。** テーブル・列・トリガー・`default`・`not null`・`check` などは原則**適用時点より後の変更にしか効かない**（例: `auth.users` の insert トリガーは既存ユーザーには発火しない）。既存行に既定値・派生行・値の穴埋めが要るなら、同じ PR に**バックフィルの DML を必ず添える**（`on conflict do nothing` 等で冪等に）。「本番に既存データが無いから不要」と判断するときも、その前提が正しいか（ローカル／既存アカウントを含む）を確認してから省く。
...
ローカルの起動・接続情報・利用方法は [`README.md`](../../README.md)「Supabase 連携（レッスン管理・ログイン）」、
