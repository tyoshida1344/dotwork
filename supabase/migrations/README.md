...
`YYYYMMDDHHMMSS_<name>.sql`。タイムスタンプ順がそのまま適用順になる。
`npx supabase migration new <name>` で生成すれば命名・順序を間違えない。
...
理由は `db push` で本番にも入ること。`on conflict (slug) do nothing` で冪等にしており、
