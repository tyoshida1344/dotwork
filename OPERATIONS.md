...
- 起こし続けたいのは本番プロジェクトなので、プレビュー用ワークフローが使う `_DEV` の Secrets とは分け、`VITE_SUPABASE_URL_PROD` / `VITE_SUPABASE_ANON_KEY_PROD` を使う。この2つをリポジトリの Actions secrets に本番プロジェクトの値で登録する（→「環境変数（本番）」）。
...
| `VITE_SUPABASE_URL_DEV` / `VITE_SUPABASE_ANON_KEY_DEV` | プレビュー用ワークフローのビルドが向く dev プロジェクト | GitHub Actions secret |
| `VITE_SUPABASE_URL_PROD` / `VITE_SUPABASE_ANON_KEY_PROD` | keep-alive が叩く本番プロジェクト（→「Supabase の一時停止対策」） | GitHub Actions secret |
